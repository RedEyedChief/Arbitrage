...
Class Content_model extends CI_Model
{
    function getNews($start=0,$end=10,$id=false)
    {
        ...
        $query = $this -> db -> query("SELECT idArticle,headerArticle, descriptionArticle, textArticle, username, dateArticle FROM SITE.ARTICLE INNER JOIN SITE.PROFILE ON Profile_idProfile = idProfile ".$where." ORDER BY dateArticle DESC LIMIT ".$start.",".$end);
    ...
    {
        $where = $id!=false?"WHERE PROFILE.idProfile=".$id:"";
        $query = $this -> db -> query("SELECT idPoll,namePoll,statePoll,username FROM SITE.POLL INNER JOIN SITE.PROFILE ON Profile_idProfile = idProfile ".$where." LIMIT ".$start.",".$end);
    ...
    function getComments($id, $start=0,$end=10)
    {
        ...
        $query = $this -> db -> query("SELECT textComment as comment,dateComment as date,userComment as username FROM COMMENT ".$where);
    ...
    function getArticle($id)
    {
        ...
        $query = $this -> db -> query("SELECT idArticle,headerArticle, descriptionArticle, textArticle, username, dateArticle, imageArticle FROM SITE.ARTICLE INNER JOIN SITE.PROFILE ON Profile_idProfile = idProfile ".$where);
